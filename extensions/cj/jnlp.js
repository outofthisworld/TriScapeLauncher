...
function parseJNLPData(jnlpData, jnlpUrl, retData)
{
	...
	var jnlpFolder = new CheerpJWebFolder("/jnlp/",loaderPath);
	...
	// Fint the right place to add the mount point
	for(var i=0;i<cheerpjFSMounts.length;i++)
	{
		if(cheerpjFSMounts[i].mountPoint < jnlpFolder.mountPoint && !jnlpFolder.mountPoint.startsWith(cheerpjFSMounts[i].mountPoint))
			continue;
		cheerpjFSMounts.splice(i,0,jnlpFolder);
		break;
	}
	if(isApplet)
	{
